...
static void SensorInterface_Command_UpdateFirmwareCallback(const DIGITALTWIN_CLIENT_COMMAND_REQUEST* commandRequest, DIGITALTWIN_CLIENT_COMMAND_RESPONSE* commandResponse, void* userInterfaceContext);
...
void SensorInterface_Command_BlinkCallback(const DIGITALTWIN_CLIENT_COMMAND_REQUEST* commandRequest, DIGITALTWIN_CLIENT_COMMAND_RESPONSE* commandResponse, void* userInterfaceContext)
{
    ...
    long interval;
    Sensor_DeserializeBlinkCommandRequest(&interval, commandRequest->requestData, commandRequest->requestDataLen);
...
void SensorInterface_Command_UpdateFirmwareCallback(const DIGITALTWIN_CLIENT_COMMAND_REQUEST* commandRequest, DIGITALTWIN_CLIENT_COMMAND_RESPONSE* commandResponse, void* userInterfaceContext)
{
    SENSOR_INTERFACE_STATE* interfaceState = (SENSOR_INTERFACE_STATE*)userInterfaceContext;
    LogInfo("SENSOR_INTERFACE: updateFirmware command invoked.");
    LogInfo("SENSOR_INTERFACE: updateFirmware request payload=<%.*s>, context=<%p>", (int)commandRequest->requestDataLen, commandRequest->requestData, interfaceState);
    ...
    //SENSOR_UPDATEFIRMWARE_firmwareRequest firmwareRequest;

    //mallocAndStrcpy_s(&firmwareRequest.firmwareUri, (char*)json_object_get_string(jsonPayload, "firmwareUri"));

    //firmwareRequest.retry = json_object_get_boolean(jsonPayload, "retry");

    SENSOR_UPDATEFIRMWARE_firmwareRequest firmwareRequest;
    Sensor_DeserializeUpdateFirmwareCommandRequest(&firmwareRequest, commandRequest->requestData, (int)commandRequest->requestDataLen);
    ...
    SENSOR_UPDATEFIRMWARE_updateFirmwareResponse response;

    DIGITALTWIN_COMMAND_RESULT commandResult = Sensor_Command_UpdateFirmware(&firmwareRequest, &response, &statusCode);

    // Serialize command response payload

    if (commandResult == DIGITALTWIN_COMMAND_OK &&
        Sensor_SerializeUpdateFirmwareCommandResponse(&response, responsePayload, MAX_MESSAGE_SIZE))
    {
        DigitalTwinClientHelper_SetCommandResponse(commandResponse, (const unsigned char*)responsePayload, statusCode);
    ...
    if (firmwareRequest.firmwareUri != NULL)
    {
        free(firmwareRequest.firmwareUri);
    }
...
void SensorInterface_ProcessCommandUpdate(const DIGITALTWIN_CLIENT_COMMAND_REQUEST* dtCommandRequest, DIGITALTWIN_CLIENT_COMMAND_RESPONSE* dtCommandResponse, void* userInterfaceContext)
{
    ...
    if (strcmp(dtCommandRequest->commandName, SensorInterface_UpdateFirmwareCommand) == 0)
    {
        SensorInterface_Command_UpdateFirmwareCallback(dtCommandRequest, dtCommandResponse, userInterfaceContext);
        return;
    }
...
DIGITALTWIN_CLIENT_RESULT SensorInterface_ReportPropertyAsync(DIGITALTWIN_INTERFACE_CLIENT_HANDLE interfaceHandle, const char* propertyName, const char* propertyData)
{
...
DIGITALTWIN_CLIENT_RESULT SensorInterface_Property_ReportState()
{
    ...
    {
        return SensorInterface_ReportPropertyAsync(appState.interfaceClientHandle, SensorInterface_StateProperty, payloadBuffer);
    }
    else
    {
...
DIGITALTWIN_CLIENT_RESULT SensorInterface_Telemetry_SendAll()
{
    ...
    Sensor_SerializeLocationTelemetry(telemetryValue, MAX_MESSAGE_SIZE);

    sprintf(combinedMessage + strlen(combinedMessage), "\"%s\":%s", SensorInterface_LocationTelemetry, telemetryValue);
